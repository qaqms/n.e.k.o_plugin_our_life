...
四件事：**数值随真实时间与作息变化**、**她每天要吃饭（口粮要你囤）**、
**她的状态会进入对话**、**你可以从面板看清这笔账**。
...
她还持有两个可自主调用的 LLM 工具：`our_life_feel`（查自己的肚子/精神/心情/身体/关系）、
...
uv run python -m pytest tests -q          # 当前 263 passed（沙箱外全绿）
...
- **她自己的判断回流成数值**（下一轮）：给她一个评估工具，让她按对话质量回传"这轮聊得开心 /
  被冷落"，把"只按发言条数算成长"的失真补上；要比 v0.2.0 更谨慎地做防刷与防崩，
  所以单独一轮。
- 病愈 / 哄好 / 生日等阶段性事件与面板叙事（与上一条一起做才有味道）
