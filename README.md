...
- **反馈闭环没有"病愈 / 哄好"这类阶段性叙事**：她判断"这轮聊得不开心"会轻微影响数值，
  但不会触发一个专门的故事事件（留到下一轮）。
...
uv run python -m pytest tests -q          # 当前 308 passed（沙箱外全绿；直连 venv 时 307 passed / 1 skipped）
...
- **病愈 / 哄好 / 生日等阶段性事件与面板叙事**（下一轮）：她判断"这轮聊得不开心"目前只轻微
  影响数值，还不成为一个故事事件；面板的"她自己的感受"块已经把读数接好了，叙事可以直接在它
  旁边长出来。
